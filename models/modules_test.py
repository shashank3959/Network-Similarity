...
##### TEST 5: Sequential, Conv2d, ReLU, AdaptiveAvgPool2d and Linear #####
x = torch.randn(2, 3, 8, 8)
layers = Sequential(
  Conv2d(3, 8, kernel_size=3, stride=1, padding=1, grad_proj=True),
  ReLU(inplace=True, grad_proj=True),
  Conv2d(8, 8, kernel_size=3, stride=2, padding=1, grad_proj=True),
  ReLU(inplace=True, grad_proj=True))
pool = AdaptiveAvgPool2d(1, grad_proj=True)
fc = Linear(8, 2, grad_proj=True)
...
# backward pass
grad = autograd.grad(y[0, 0], 
  list(layers.parameters()) + list(fc.parameters()))
...
# random vectors
conv1_weight_rv, conv1_bias_rv = layers[0].weight_rv, layers[0].bias_rv
conv2_weight_rv, conv2_bias_rv = layers[2].weight_rv, layers[2].bias_rv
fc_weight_rv, fc_bias_rv = fc.weight_rv, fc.bias_rv
